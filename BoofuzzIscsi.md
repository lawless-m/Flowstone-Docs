...
Built on the boofuzz framework. Supports full-session fuzzing or a faster login-only mode for initial testing, exposes boofuzz's web interface on localhost:26000 to watch progress, and carries its own PDU definitions for login requests, SCSI commands, text/nop/logout opcodes. Companion to [[VoE]] (the Venti-over-Ethernet target this fuzzer can hammer) and [[IscsiCrate]] (the pure-Rust target library). Together the three form the iSCSI cluster in this corpus.
